  ...
  return <div className="details-enter">
      ...
      <section className="mt-12">
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/[.08] bg-[#111414]">
          <div className="min-w-[720px]"><div className="grid grid-cols-[minmax(180px,1fr)_90px_100px_90px_150px_90px] gap-4 border-b border-white/[.07] px-5 py-3 text-[10px] font-semibold uppercase tracking-[.16em] text-white/30"><span>Project</span><span>Sessions</span><span>Tokens</span><span>Tools</span><span>Top model</span><span className="text-right">Active</span></div>
...
function ProjectRow({ project, last }: { project: Profile["projects"][number]; last: boolean }) { return <div className={`grid grid-cols-[minmax(180px,1fr)_90px_100px_90px_150px_90px] items-center gap-4 px-5 py-3 text-sm ${last ? "" : "border-b border-white/[.06]"}`}><span className="truncate font-medium">{project.name}</span><span className="font-mono text-xs text-white/45">{integer(project.sessions)}</span><span className="font-mono text-xs">{compact(project.tokens)}</span><span className="font-mono text-xs text-white/45">{compact(project.toolCalls)}</span><ModelSplit models={project.models} /><span className="text-right text-xs text-white/30"><RelativeTime value={project.lastActive} /></span></div>; }
