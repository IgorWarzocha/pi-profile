...
type Profile = {
  generatedAt: string;
  profile: { name: string; handle: string; avatarUrl: string; links: { x: string; github: string; website: string; linkedin: string } };
...
export function App() {
  ...
  return <main className="min-h-screen bg-[#0d0f0f] text-[#f2f1eb] selection:bg-[#8eb7ff] selection:text-black">
    ...
    <div className="mx-auto max-w-[1180px] px-5 pb-20 pt-8 sm:px-8 lg:px-10">
      ...
      <section className="mt-12">
        <SectionHeading title="Language" aside={<span className="text-xs text-white/30">{integer(p.languageSummary.userMessages)} messages</span>} />
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <SignalCard title="Friction" count={p.languageSummary.friction} messageCount={p.languageSummary.userMessages} values={p.language.friction} accent="#ef8f77" />
        </div>
        <div className="mt-4 flex items-center justify-between rounded-2xl border border-white/[.08] bg-[#111414] px-5 py-4"><span className="font-medium">Ragio</span><span className="font-mono text-xl text-[#ef8f77]">{p.languageSummary.ragio.toFixed(2)}</span></div>
      </section>
      ...
      <footer className="mt-14 flex flex-col justify-between gap-2 border-t border-white/[.08] pt-5 text-xs text-white/30 sm:flex-row"><span>Updated {relativeTime(p.generatedAt)}</span><span>Cache read {percent(t.cacheReadShare)} · {money(t.cost)} reported cost</span></footer>
    </div>
  </main>;
}
...
function MetricTabs({ value, onChange }: { value: HeatMetric; onChange: (metric: HeatMetric) => void }) {
  ...
  return <div className="flex max-w-full gap-3 overflow-x-auto">{tabs.map((tab) => <button key={tab} onClick={() => onChange(tab)} className={`whitespace-nowrap border-b px-0.5 py-1 text-[11px] transition focus:outline-none focus:ring-2 focus:ring-[#8eb7ff]/50 ${value === tab ? "border-[#8eb7ff] text-white" : "border-transparent text-white/30 hover:text-white/65"}`}>{tab === "toolCalls" ? "Tools" : titleCase(tab)}</button>)}</div>;
}
function SocialLinks({ profile }: { profile: Profile["profile"] }) {
  const links = [
    [profile.links.x, `X ${profile.handle}`],
    [profile.links.github, "GitHub @IgorWarzocha"],
    [profile.links.website, "howaboua.dev"],
    [profile.links.linkedin, "LinkedIn igorwarzocha"],
  ];
  return <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm text-white/40">{links.map(([href, label]) => <a key={href} href={href} target="_blank" rel="noreferrer" className="transition hover:text-white focus:outline-none focus:ring-2 focus:ring-[#8eb7ff]/50">{label}</a>)}</div>;
}
...
function SectionHeading({ title, aside }: { title: string; aside?: any }) { return <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><h2 className="text-xl font-medium tracking-[-.025em]">{title}</h2>{aside}</div>; }
